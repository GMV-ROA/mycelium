...
# PRESET_FILE  = dir_path+"/../cfg/d4xx-default.json"
PRESET_FILE  = "/home/robot/packages/mycelium/cfg/d4xx-default.json"
...
parser.add_argument('--save_obstacle_distances',type=int,
                    help="Save obstacle distance data")
parser.add_argument('--save_colour_frames',type=int,
                    help="Save colour frames")
parser.add_argument('--save_depth_frames',type=int,
                    help="Save depth frames")
...
save_obstacle_distances = args.save_obstacle_distances
save_colour_frames = args.save_colour_frames
...
timestr = str(int(time.time()))#datetime.datetime.now().strftime("%H%M%S")
obstacle_distance_file = None
colour_video = None
depth_video = None

if save_dir is None:
    save_dir = "rs_data/"
if save_obstacle_distances == 1:
    filename = save_dir + "d4xx_OD_"+ timestr +".log"
    obstacle_distance_file = open(filename, "a")

if save_colour_frames == 1:
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    colour_video = cv2.VideoWriter(save_dir + 'd4xx_colour_'+ timestr +'.avi', fourcc, 1,(COLOR_WIDTH, COLOR_HEIGHT))

if save_depth_frames == 1:
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    depth_video = cv2.VideoWriter(save_dir + 'd4xx_depth_'+ timestr +'.avi', fourcc, 1,(DEPTH_WIDTH, DEPTH_HEIGHT))
...
try:
    while not main_loop_should_quit:
        ...
        if obstacle_distance_file:
            timestamp = str(time.time())
            write_str = "[" + timestamp + "] {}".format(distances)
            obstacle_distance_file.write(write_str + "\n")

        if colour_video:
            img = np.asanyarray(color_frame.get_data())
            colour_video.write(img)

        if depth_video:
            img = np.asanyarray(depth_frame.get_data())
            img8bit = cv2.convertScaleAbs(img, alpha=0.03)
            depth_video.write(img8bit)
